...
// ============================================================
// Helpers - Self Registration + Teacher Approval
// ============================================================
...
function buildGroupName(level, year, room) {
  ...
  if (cleanRoom.includes('/')) {
    return cleanLevel && cleanYear
...
async function getStudentByLineUserId(lineUserId) {
  ...
  const result = await pool.query(
    `SELECT id, student_code, name, group_name, education_level,
     ...
     FROM students WHERE line_user_id = $1 LIMIT 1`,
    [lineUserId]
  );
  return result.rows[0] || null;
}

function assertStudentApproved(student) {
  const status = normalizeApprovalStatus(student.approval_status);
  if (status === 'pending') {
    return { ok: false, status: 403, error: 'รอครูยืนยันการลงทะเบียน กรุณารอสักครู่' };
  }
  if (status === 'rejected') {
    return { ok: false, status: 403, error: 'การลงทะเบียนถูกปฏิเสธ กรุณาลงทะเบียนใหม่หรือติดต่อครู' };
  }
  if (!student.is_active) {
    return { ok: false, status: 403, error: 'บัญชีนักเรียนถูกระงับ กรุณาติดต่อครู' };
  }
  return { ok: true };
}

// ============================================================
// POST /api/liff/register - Self-Registration (รอครูยืนยัน)
// ============================================================
router.post('/register', async (req, res) => {
  try {
    const payload = normalizeStudentPayload(req.body);
    const { lineUserId, studentCode, prefix, firstName, lastName, level, year, room, major } = payload;

    if (!lineUserId) {
      return res.status(400).json({ success: false, error: 'ไม่พบ LINE User ID' });
    }
    if (!studentCode) {
      return res.status(400).json({ success: false, error: 'กรุณากรอกรหัสนักเรียน' });
    }
    if (!firstName || !lastName) {
      return res.status(400).json({ success: false, error: 'กรุณากรอกชื่อและสกุล' });
    }
    if (!level || !year || !room || !major) {
      return res.status(400).json({ success: false, error: 'กรุณากรอกระดับ ชั้นปี กลุ่ม/ห้อง และสาขาวิชาให้ครบ' });
    }

    await ensureStudentRegistrationColumns();
    const fullName = buildStudentName(prefix, firstName, lastName);
    const groupName = buildGroupName(level, year, room);
    const classYear = String(year || '').replace(/^ปี\s*/i, '').trim() || null;
    ...
    if (existing.rows.length > 0) {
      ...
      const currentStatus = normalizeApprovalStatus(row.approval_status);
      ...
      if (currentStatus === 'approved' && row.is_active === true && row.line_user_id === lineUserId) {
        ...
        return res.json({
          ...
          message: 'คุณลงทะเบียนและได้รับการยืนยันแล้ว',
          student: formatStudentResponse(approved)
        });
      }
    }
    ...
    return res.json({
      ...
      message: 'ส่งคำขอลงทะเบียนแล้ว รอครูยืนยัน',
...
// ============================================================
// GET /api/liff/profile/:lineUserId
// ============================================================
router.get('/profile/:lineUserId', async (req, res) => {
  try {
    const { lineUserId } = req.params;
    const row = await getStudentByLineUserId(lineUserId);
    ...
    if (approvalStatus === 'rejected') {
      return res.json({
        ...
        message: 'การลงทะเบียนถูกปฏิเสธ กรุณาลงทะเบียนใหม่'
      });
    }
    ...
    if (approvalStatus === 'pending') {
      return res.json({
        registered: true,
        approvalStatus: 'pending',
        message: 'รอครูยืนยันการลงทะเบียน',
        student: formatStudentResponse(row),
...
router.post('/check-in', async (req, res) => {
  try {
    ...
    // ---- 1. ตรวจสอบนักเรียน ----
    const studentRow = await getStudentByLineUserId(lineUserId);
    if (!studentRow) {
      return res.status(404).json({ success: false, error: 'ไม่พบข้อมูลนักเรียน กรุณาลงทะเบียนก่อน' });
    }
    const approvalCheck = assertStudentApproved(studentRow);
    if (!approvalCheck.ok) {
      return res.status(approvalCheck.status).json({ success: false, error: approvalCheck.error });
    }
...
router.post('/face/register', async (req, res) => {
  try {
    ...
    await pool.query(
      ...
      [JSON.stringify({ student_name: studentRow.name }), studentId]
    );
